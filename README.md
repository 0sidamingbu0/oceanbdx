...
零速闭环。腿关节只需大致接近共享 neutral 姿态；精确脚距、前后错位和相对 yaw 仅用于诊断，
不再作为切换硬门槛，避免站立策略无法主动修脚时永久卡住。不读取接触力、世界足端位置、
CoM 或真实 base velocity。回正期间屏蔽新的 head/torso 命令；
...
head 命令继续保留。脖子最终位置目标在 FOH/低通后再限制为最大 `1.0rad/s`、`12.0rad/s²`，
大幅头部动作会快速但平滑地加速并提前制动。standing 身体的高度、pitch、yaw、roll 命令也使用
独立的二阶速度/加速度限制，避免手柄输入瞬间跳到姿态极限。`STAND_UP→RL_STAND`
...
使用完整域的 1/2：`±0.01m / ±0.25 / ±0.50 / ±0.30rad`。手柄映射按当前 policy 限幅；站转走
...
IMU 和腿 q/dq。满足条件后自动完成一次切换，不会循环重启回正流程
或刷超时日志。
